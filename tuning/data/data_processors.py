...
class DataPreProcessor:
    ...
    def process_dataset_configs(
        ...
        try:
            ...
            logger.error(
                "NCCL communication timeout detected. "
                "Try enabling streaming mode to avoid large collective operations. "
                "Or increase `timeout` parameter in training arguments"
            )
            raise RuntimeError(
                "Dataset processing failed due to NCCL timeout. Use streaming or increase `timeout` in training args instead."
            ) from e
